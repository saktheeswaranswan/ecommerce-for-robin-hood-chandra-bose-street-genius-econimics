...
while True:
    ...
    if len(face_data) > 0:
        ...
        if elapsed_time.total_seconds() >= VIDEO_DURATION:
            ...
            output_video_file = cv2.VideoWriter(
                output_file, cv2.VideoWriter_fourcc(*"mp4v"), FRAME_RATE, (frame.shape[1], frame.shape[0]))
            ...
            # Reset the start time for the next video clip
    ...
    if len(face_data) > 0:
        if output_audio.empty():
            output_audio_start_time = datetime.datetime.now()
